...
const HomeScreenContent = ({ feedings, sleeps, diapers, walks, tasks, insights }: any) => {
  ...
  const addSyntheticSleep = async () => {
    const recentSleeps = sleeps.slice(0, 5); 
    if (recentSleeps.length === 0) return;
    const avgDuration = recentSleeps.reduce((acc: any, s: any) => acc + s.duration_seconds, 0) / recentSleeps.length;
      ...
      >
        ...
        >
          ...
          {(widgetType === 'sleep' || widgetType === 'sleep_soon') && lastSleep && (Date.now() - getSleepEndMs(lastSleep) > wakeWindowMs * 1.5) && (
            ...
            >
              <Text style={{ fontFamily: 'Nunito_700Bold', fontSize: 12, color: '#FFFFFF' }}>Забыли записать сон?</Text>
            </TouchableOpacity>
          )}
        </TouchableOpacity>
        ...
        {latestInsight ? (
          <TouchableOpacity 
            ...
            activeOpacity={0.9}
            style={{ marginHorizontal: 16, marginBottom: 16, backgroundColor: latestInsight.type === 'success' ? '#ECFDF5' : latestInsight.type === 'warning' ? '#FEF2F2' : '#F5F7FF', borderRadius: 20, padding: 16, borderColor: latestInsight.type === 'success' ? '#D1FAE5' : latestInsight.type === 'warning' ? '#FEE2E2' : '#E0E7FF', borderWidth: 1, shadowColor: '#6366F1', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 16, elevation: 3 }}
          >
             <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                 <View style={{ width: 28, height: 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: latestInsight.type === 'success' ? '#10B981' : latestInsight.type === 'warning' ? '#EF4444' : '#4F46E5' }}>
                   <Bot size={15} color="#FFFFFF" />
                 </View>
                 <Text style={{ fontFamily: 'Nunito_800ExtraBold', fontSize: 13, color: latestInsight.type === 'success' ? '#065F46' : latestInsight.type === 'warning' ? '#991B1B' : '#3730A3', flex: 1 }}>{latestInsight.insight_title || "Еженедельный отчет"}</Text>
             </View>
             <Text style={{ fontFamily: 'Nunito_700Bold', fontSize: 14, color: '#1A1A2E', lineHeight: 20 }}>{latestInsight.short_text}</Text>
          </TouchableOpacity>
        ) : (
          <TouchableOpacity 
            onPress={() => DeviceEventEmitter.emit('openAIBubble')}
            activeOpacity={0.9}
            style={{ marginHorizontal: 16, marginBottom: 16, backgroundColor: '#F5F7FF', borderRadius: 20, padding: 16, borderColor: '#E0E7FF', borderWidth: 1, shadowColor: '#6366F1', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 16, elevation: 3 }}
          >
             <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                 ...
                 <Text style={{ fontFamily: 'Nunito_800ExtraBold', fontSize: 11, color: '#4F46E5', textTransform: 'uppercase' }}>AI-ИНСАЙТЫ ОТКЛЮЧЕНЫ</Text>
             </View>
             <Text style={{ fontFamily: 'Nunito_700Bold', fontSize: 13, color: '#1E1B4B', lineHeight: 20 }}>Скоро здесь еженедельно будут появляться персонализированные полезные советы от AI-педиатра по графику малыша.</Text>
          </TouchableOpacity>
        )}
